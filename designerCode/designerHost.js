...
window.virtual.browserExperience = {
    allowNavigate:false
};

window.virtual.idPathPair = {};

ipcRenderer.on('designer', (e,a) => {

    let message = JSON.parse(a);

    if (message.action == 'REQUEST_STYLES'){
        ipcRenderer.sendToHost(message.id,virtual.getCSSString())
    }
    else if (message.action == 'INSERT_ELEM'){
        toolbox.addTool(message.data);
    }
    else if (message.action == 'REQUEST_DOCUMENT'){
        ipcRenderer.sendToHost(message.id,document.documentElement.outerHTML)
    }
    else if (message.action == 'EXEC_THIS'){
        eval(message.data);
    }
    else if (message.action == 'REQ_THIS'){
        window[message.data.name] = require(message.data.module);
    }
    else if (message.action == 'SYNC_TOOLBOX'){
        window.toolbox = {};
        window.creator = {};
        window.creator.controls = {};
        window.creator.controls.core = {};

        //noinspection JSAnnotator
        window.toolbox = {};
        window.toolbox.addToolX = function(name){
            return nativejsx.parseSync(name, {
                declarationType: 'var',
                variablePrefix: '$$'
            })
        };
        window.toolbox.addTool = function (name) {
            let pathC = path.join( remote.getGlobal('editorPath') , path.join('NSControls', name+'.jsx') );
            let toolName = name;
            virtual.updateHistory();
            ...
            if (typeof HTMLElement.prototype.setStyles !== 'function'){
                HTMLElement.prototype.setStyles = function(obj){
                    ...
                    if (typeof obj === "string"){
                        this.style.setProperty(obj.split(':')[0],obj.split(":")[1]);
                    }
                    else{
                        for (let p in obj){
                            this.style.setProperty(p,obj[p]);
                        }
                    }
                }
            }
            else{
                console.log(HTMLElement.prototype.setStyles);
            }

            let leElement = creator.controls.core[toolName].build();
            if (leElement.hasAttribute('id') == false) leElement.id = virtual.componentGUID.GetNew();
            $('.marker').replaceWith( leElement );
            setTimeout(virtual.updateHistory,600);
        };
    }
    else if (message.action == 'NEW_TARGET'){
        if (virtual.browserExperience.allowNavigate == true) return;
        let e = message.data;
        designerHost.wrappers.mouseclick(e);
    }
    else if (message.action == 'NEW_HOVER_TARGET'){
        if (virtual.browserExperience.allowNavigate == true) return;
        let e = message.data;
        designerHost.wrappers.mousemove(e);
    }
    else if (message.action == 'NO_TARGET'){
        document.body.removeChild(designerHost.selector);
        designerHost.selector = undefined;
    }
    else if (message.action == 'NO_HOVER_TARGET'){
        document.body.removeChild(designerHost.marker);
        designerHost.marker = undefined;
    }
    else if (message.action == 'TOGGLE_BE_ALLOWNAVIGATE'){
        if (message.data =! 'false' && message.data != 'true'){
            virtual.browserExperience.allowNavigate = false && virtual.browserExperience.allowNavigate;
        }
        else{
            virtual.browserExperience.allowNavigate = Boolean(message.data);
        }
    ...
    else if (message.action == 'REQUEST_ALLOWNAVIGATE'){
        ipcRenderer.sendToHost(message.id,virtual.browserExperience.allowNavigate)
    }
    else if (message.action == 'SET_STYLE'){
        let parsedCSS = {};
        parsedCSS.body = message.data.value.split('{')[1].split('}')[0].split(';');

        let mytarget = $(message.data.target);
        if (mytarget[0] == undefined) return;

        if (mytarget[0].id == '' || mytarget[0].id == undefined) {
            mytarget[0].id = message.id;
        }
        else{
            mytarget = $('#'+mytarget[0].id);
            ipcRenderer.sendToHost('correctFocusItem',mytarget[0].id);
        }

        virtual.css[ mytarget[0].id ] = {};

        for (let index in parsedCSS.body){
            virtual.css[ mytarget[0].id ][parsedCSS.body[index].split(':')[0]] = parsedCSS.body[index].split(':')[1];
            mytarget.css(parsedCSS.body[index].split(':')[0],parsedCSS.body[index].split(':')[1]);
        }
        virtual.updateHistory();
    }
    else if (message.action == 'CHANGE'){
        if (message.data == 'active'){
            virtual.historyRecording = true;
        }
    }
...
ipcRenderer.on('system', (e,a) => {
    ipcRenderer.sendToHost('status',['received'])
});

window.virtual.getCSSString = function(){
    return "";
};

class virtHist extends Array{
    constructor(lim=10){
        super();
        this.limit = lim;
    }
    push(x,ifUnique=true){
        if ( ifUnique == true && this.sameAsLast(x) == true) return false;
        super.push(x);
        if (this.length > this.limit) this.splice(0,1);
        return this;
    }
    sameAsLast(item){
        if (this.length > 0){
            let dd = new diffDOM();
            let diff = dd.diff(item[0], this[this.length-1][0]);
            return (diff.length > 0);
        }
        return false;
    }
    getLast(jump=1){
        let res = this[this.length-jump];
...
window.virtual.history = new virtHist();
window.virtual.historyRecording = false;
window.virtual.undoHistory = function(){
    if (virtual.historyRecording == false) return;
    // let histMark, histSel;
    // if (designerHost.marker != undefined) {
    //     histMark = $(designerHost.marker).detach();
    //     designerHost.marker = undefined;
    // }
    // if (designerHost.selector != undefined) {
    //     histSel = $(designerHost.selector).detach();
    //     designerHost.selector = undefined;
    // }
    let prevVers = virtual.history.getLast(2)[0];
    let dd = new diffDOM();
    let diff = dd.diff(document.body, prevVers);
    dd.apply(document.body, diff);
};

window.virtual.updateHistory = function(){
    if (virtual.historyRecording == true) virtual.history.push( $(document.body).clone(true,true) ,false);
};

window.virtual.componentGUID = new (function() {
...
designerHost.wrappers = {};
designerHost.wrappers.mousemove = function(e){
    let xd = document.elementFromPoint(e.x,e.y);
    if (xd.tagName == 'HTML') xd = document.body;

    if(xd.hasAttribute('e') == true || xd.tagName == 'BODY'){
        let myNewName = xd.localName;
        if (xd.hasAttribute('e')) myNewName = xd.getAttribute('e');
        designerHost.putMarker(xd.offsetLeft,xd.offsetTop, xd.clientWidth,xd.clientHeight,myNewName,xd)
    }
    else{

        if (xd.tagName == 'HTML') xd = document.body;
        while(xd.tagName != 'BODY' && xd.hasAttribute('e') == false){
            xd = xd.parentNode;
        }
        let myNewName = xd.localName;
        if (xd.hasAttribute('e')) myNewName = xd.getAttribute('e');
...
designerHost.wrappers.mouseclick = function(e){
    let xd = document.elementFromPoint(e.x,e.y);
    if (xd.tagName == 'HTML') xd = document.body;
    ...
    if(xd.hasAttribute('e') == true || xd.tagName == 'BODY'){
        let myNewName = xd.localName;
        if (xd.hasAttribute('e')) myNewName = xd.getAttribute('e');
        designerHost.putSelector(xd.offsetLeft,xd.offsetTop, xd.clientWidth,xd.clientHeight,myNewName,xd)
    }
    else{
...
designerHost.importSupportStyle = function(src){
    let style = document.createElement('link');
    style.href = src;
    style.rel = 'stylesheet';
    document.head.appendChild(style);
    designerHost.addHelper();
    ...
    window.setInterval(function(){
        $('img').each(function() {
            if ((typeof this.naturalWidth != "undefined" &&
                ...
                let photo = new Unsplash.UnsplashPhoto();
                photo.of(['forest','summer','water']).width(1024);
                $(this).attr('src', photo.fetch());
            }
        });
    },200);
};

designerHost.addHelper = function(){
    window.getPath = function (m) {
        m = m[0];
        let selector = new CssSelectorGenerator;
        let val = selector.getSelector(m);

        return val;
    };
};

designerHost.try = function (fn,me,complete=function(successful){}) {
    try{
        fn.call(me);
        complete(true);
    }
...
designerHost.putMarker = function(x,y,w,h,title,elem){
    let pos='absolute';
    let orig = elem;
    if (w==0 || h == 0){
        w = elem.offsetWidth;
    ...
    if (designerHost.marker != undefined) {
        designerHost.try(function(){
            $(designerHost.marker).remove();
        },this);
        designerHost.marker = undefined;
    }
    ...
    $(elem).find('[c="true"]').each(function() {
        if( !this.firstChild || this.firstChild.nodeType !== 1  ) {
            let levelsFromThis = $(this).parentsUntil(elem).length;
    ...
    elem = (deepest || undefined);

    if ( $(elem).has('[c="true"]') !== true && $(elem).is('[c="true"]') === false) {
        if (elem === undefined) elem = orig;
        elem = ($(elem).find('[c="true"]')[0] || $('body')[0]);
    }

    title = elem.className;

    designerHost.marker = document.createElement('div');
    designerHost.marker.className = 'marker';
    designerHost.marker.style.left = x+"px";
    designerHost.marker.style.top = y+"px";
    designerHost.marker.style.width = w+"px";
    designerHost.marker.style.height = h+"px";
    let isFixed = (function isFixed(eem){
        do{
            if(getComputedStyle(eem).position == 'fixed') return true;
        }while(eem = eem.offsetParent);
        return false;
    })(elem);
    designerHost.marker.style.position = ( isFixed ? 'Fixed' : pos);
    designerHost.marker.setAttribute('title',title);
    elem.appendChild(designerHost.marker);
};


designerHost.putSelector = function(x,y,w,h,title,elem){
    let pos='absolute';
    ...
    if (designerHost.selector != undefined) {
        ...
        designerHost.selector = undefined;
        ipcRenderer.sendToHost('noFocusItem','');
    }
    designerHost.selector = document.createElement('div');
    designerHost.selector.className = 'selector';
    designerHost.selector.style.left = x+"px";
    designerHost.selector.style.top = y+"px";
    designerHost.selector.style.width = w+"px";
    designerHost.selector.style.height = h+"px";
    let isFixed = (function isFixed(eem){
        do{
            if(getComputedStyle(eem).position == 'fixed') return true;
        }while(eem = eem.offsetParent);
    ...
    designerHost.selector.style.position = ( isFixed ? 'Fixed' : pos);
    designerHost.selector.setAttribute('title',title);
    document.body.appendChild(designerHost.selector);


    if (elem.id != '') ipcRenderer.sendToHost('focusItem','#'+elem.id);
    else ipcRenderer.sendToHost('focusItem',window.getPath([elem]));

    window.onresize = function(){
        designerHost.putSelector(elem.offsetLeft,elem.offsetTop, elem.clientWidth,elem.clientHeight, elem.localName,elem);
        if (designerHost.marker != undefined) {
            document.body.removeChild(designerHost.marker);
            designerHost.marker = undefined;
        }
    ...
    if ( virtual.css[elem.id] != undefined){
        ipcRenderer.sendToHost('getRecord',{
            elementID:elem.id,
            styles:virtual.css[elem.id]
        });
    }
};
...
window.onbeforeunload = function(e) {
    if (!virtual.browserExperience.allowNavigate){
       return false;
    }
};